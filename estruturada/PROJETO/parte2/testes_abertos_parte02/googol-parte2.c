...
/* dado uma string, um vetor de inteiro e o tamanho da string, esse vetor copia todos os elementos da string para o final do vetor de inteiros num[TAM_MAX]. Ex: '12345' -> [0,...,0,0,0,0,0,1,2,3,4,5] */
...
/* conta os digitos de um vetor sem contar os primeiros 0 (da esquerda pra direita) e retorna a quantidade desses digitos, exemplo [0,0,0,0,0,1,2,3,4,5,0] -> 6 */
...
/* dado dois vetores, esta funcao transforma num2 em num1, ou seja, todas posicoes de num2 se torna igual as respectivas posicoes de num1*/
...
/* dado um vetor e um inteiro como parametro, esta funcao adiciona o inteiro dado na ultima posicao do vetor */
void adicionaInteiro(int vetor[], int numero);

/* dado um vetor, um inteiro e o tamanho do vetor como parametro, esta funcao transforma o vetor dado por um vetor com apenas qtdeDigitos digitos significativos. Ex: dividendo=[1,2,3,4,5], qtdeDigitos=2 o vetor se transforma em: [1,2] */
void incrementaDividendo(int dividendo[], int qtdeDigito, int tamanhoDiv);

/* dado dois vetores como parâmetro, essa funcao retorna 1 caso os vetores forem iguais (em modulo) ou 0 se forem diferentes */
int verficaIgual(int num1[], int num2[]);

/* dado 2 vetores, seus tamanhos e um caractere, esta funcao calcula a divisao num1/num2 e armazena o resto da divisao no num1 caso o caractere divResto seja '%' ou armazena o resultado no num1 caso o divResto seja '/' */
void divisao(int num1[], int num2[], int tamanho1, int tamanho2, char diviResto);
...
int verificaIgual(int num1[], int num2[]){
    int i;
    for(i=1; i<TAM_MAX; i++){
        if(num1[i]!=num2[i]){
            return 0;
...
void divisao(int num1[], int num2[], int tamanho1, int tamanho2, char diviResto){
    int tamDivid,multiplicador,tamanhoMult,jaDividiu=0;
    int sinalResu,sinalRest,indice,dividMaior=0,maior,resto;
    int dividendo[TAM_MAX],resultado[TAM_MAX],multiplicado[TAM_MAX];

    zeraVetor(multiplicado,TAM_MAX);
    zeraVetor(resultado,TAM_MAX);
    ...
    if(verificaIgual(num1,num2)){
        zeraVetor(num1,TAM_MAX);
        if(diviResto=='%'){
            num1[0]=sinalRest;
        }else{
            num1[TAM_MAX-1]=1;
    ...
    if(num2[TAM_MAX-1]==0 && num2[TAM_MAX-2]==1 && tamanho2==2){
        if(diviResto=='%'){
            resto=num1[TAM_MAX-1];
            zeraVetor(num1,TAM_MAX);
            num1[TAM_MAX-1]=resto;
            num1[0]=sinalRest;
        }else{
            incrementaDividendo(num1,tamanho1-1,tamanho1);
            num1[0]=sinalResu;
        }
        return;
    }

    copiaVetor(num1,dividendo);
    incrementaDividendo(dividendo,tamanho2-1,tamanho1);
    tamDivid=tamanho2-1;
    indice=TAM_MAX-tamanho1+tamanho2-1;

    while(indice<TAM_MAX){
        while(!dividMaior){
          adicionaInteiro(dividendo,num1[indice]);
          indice++;
          tamDivid++;
          maior=maiorMenor(dividendo,num2,tamDivid,tamanho2);
          if(maior>=0){
            dividMaior=1;
          }else{
              if(jaDividiu){
                adicionaInteiro(resultado,0);
              }
          }
          if(indice>TAM_MAX){
              break;
        ...
        if(indice>TAM_MAX){
          ...
          while(dividMaior){
              jaDividiu=1;
            for(multiplicador=9; multiplicador>0; multiplicador--){

              copiaVetor(num2,multiplicado);
              multiplicacaoSimples(multiplicado,multiplicador);
              tamanhoMult=tamanhoVetor(multiplicado,-1);
              maior=maiorMenor(dividendo,multiplicado,tamDivid,tamanhoMult);

              if(maior>=0){
                adicionaInteiro(resultado,multiplicador);
                tamanhoMult=tamanhoVetor(multiplicado,-1);
                subtrai(dividendo,multiplicado,tamDivid,tamanhoMult);
                tamDivid=tamanhoVetor(dividendo,-1);
                maior=maiorMenor(dividendo,num2,tamDivid,tamanho2);
                if(maior<0){
                  dividMaior=0;
                }
    ...
    if(diviResto=='%'){
        copiaVetor(dividendo,num1);
        num1[0]=sinalRest;
    }else{
        copiaVetor(resultado,num1);
        num1[0]=sinalResu;
    }
}
  ...
  int vetor1[TAM_MAX], vetor2[TAM_MAX],*maior,*menor;
    ...
    while(1){
      ...
      switch(operacao[0]){
        ...
        case('%'):
          if(menor==vetor1){
            imprimeNumerao(menor,tamanhoMenor);
          }else{
            divisao(maior,menor,tamanhoMaior,tamanhoMenor,'%');
            imprimeNumerao(maior,-1);
          }
          break;
