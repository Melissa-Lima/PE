...
/*void divisao(int num1[], int num2[], int tamanho1, int tamanho2, char diviResto){
    int tamanhoDividendo,divisivel=1,multiplicador,tamanhoRes=0,digitosRestantes,tamanhoAux,sinal,qtdeDigito,indice;
    int dividendo[TAM_MAX],resultado[TAM_MAX],resto[TAM_MAX],aux[TAM_MAX];
    ...
    qtdeDigito=tamanho2;
    indice=TAM_MAX-tamanho1+qtdeDigito;
    incrementaDividendo(dividendo,qtdeDigito,tamanho1);
    copiaVetor(num1,dividendo);
    while(divisivel){
        copiaVetor(num2,aux);
        digitosRestantes=tamanho1-qtdeDigito;
        tamanhoDividendo=tamanho1-digitosRestantes;
        qtdeDigito++;
        tamanhoAux=tamanhoVetor(aux,-1);
        if(!digitosRestantes){
            divisivel=0;
        }
        if(tamanhoDividendo>=tamanhoAux){
            for(multiplicador=9; multiplicador>=1; multiplicador--){
                copiaVetor(num2,aux);
                multiplicacaoSimples(aux,multiplicador);
                tamanhoAux=tamanhoVetor(aux,-1);
                if(tamanhoAux<=tamanhoDividendo){
                    adicionaInteiro(resultado,multiplicador);
                    tamanhoRes++;
                    subtrai(dividendo,aux,tamanhoDividendo,tamanhoAux);
                    if(digitosRestantes!=0){
                        adicionaInteiro(dividendo,num1[indice]);
                    }
                    if(!divisivel){
                        copiaVetor(dividendo,resto);
                    }
                    break;
                }
            }
        }else{
            incrementaDividendo(dividendo,1,tamanho1);
        }
    }

    resultado[0]=sinal;
    if(diviResto=='%'){
        copiaVetor(resto,num1);
    }else{
...
void divisao(int num1[], int num2[], int tamanho1, int tamanho2, char diviResto){
    int tamanhoDividendo,divisivel=1,multiplicador,tamanhoRes=0,digitosRestantes,tamanhoAux,sinal,qtdeDigito,indice,jaDividiu;
    int dividendo[TAM_MAX],resultado[TAM_MAX],resto[TAM_MAX],aux[TAM_MAX];
    ...
    while(divisivel){
        if(!jaDividiu){
            ...
            maior=maiorMenor(dividendo,aux,tamanhoDividendo,tamanhoAux);
            if(maior>=0){
        ...
        if(tamanhoDividendo>=tamanhoAux){
            ...
            for(multiplicador=9; multiplicador>=1; multiplicador--){
                ...
                multiplicacaoSimples(aux,multiplicador);
                tamanhoAux=tamanhoVetor(aux,-1);
                maior=maiorMenor(aux,dividendo,tamanhoAux,tamanhoDividendo);
                if(maior<=0){
                    adicionaInteiro(resultado,multiplicador);
                    tamanhoRes++;
                    subtrai(dividendo,aux,tamanhoDividendo,tamanhoAux);
...
int main(){
   ...
   divisao(vetor1,vetor2,tamanho1,tamanho2,'/');
